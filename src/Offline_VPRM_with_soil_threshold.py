...
def preprocess_vprm_for_morris(sitename, year, lat, lon, tile, input_origin = 'ERA5', level = 1):
    ...
    era5_sm_levels = [0.035,0.175,0.64,1.775]
    wrf_sm_levels = [0.05,0.25,0.70,1.5]

    if level == 1:
        lev1 = '1'
        lev2 = '2'
        uplev = era5_sm_levels[0]
        lowlev = era5_sm_levels[1]
    elif level == 2:
        lev1 = '2'
        lev2 = '3'
        uplev = era5_sm_levels[1]
        lowlev = era5_sm_levels[2]
    elif level == 3 or level == 4:
        lev1 = '3'
        lev2 = '4'
        uplev = era5_sm_levels[2]
        lowlev = era5_sm_levels[3]
    
    coefUP = (lowlev - wrf_sm_levels[level-1])/(lowlev- uplev)
    coefDWN = (wrf_sm_levels[level-1] - uplev)/(lowlev- uplev)
    ...
    if input_origin == 'ERA5':
        ...
        for month in range(12):
            if input_origin == 'ERA5' or (input_origin == 'OBS' and (not OBS_SW_IN  or not OBS_TA)):
                ...
                up_swvl_era5 = np.array(met_nc.variables['swvl'+lev1])
                up_swvl_era5 = up_swvl_era5[:,::-1,:]
                
                down_swvl_era5 = np.array(met_nc.variables['swvl'+lev2])
                down_swvl_era5 = down_swvl_era5[:,::-1,:]
                swvl_era5 = coefUP*up_swvl_era5  + coefDWN*down_swvl_era5
                
                swvl_out = factorNE*swvl_era5[:,JSW + 1, ISW + 1] + factorNW*swvl_era5[:,JSW + 1, ISW] + factorSE*swvl_era5[:,JSW, ISW + 1] + factorSW*swvl_era5[:,JSW, ISW]
