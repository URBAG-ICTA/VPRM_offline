...
def extract_wrf_times_from_evi(evi, start_mdy, start_hrs, nhrs, evi_times, delt):
    ...
    fjdays = jday0 - (julian(1,1, start_mdy.year)-1) + (start_hrs/24) + np.arange(0, ((nhrs-1)/delt)+1)/(24/delt)
...
def offlineVPRM(Temp, Rad, start_mdy, start_hrs,  evi, lswi, vegFracMap,
                      ...
                      landcov="SingleEviMap", returnLongList=True, delt=1, evi_times=np.linspace(2005041,2005305,8),
    ...
    if landcov =='DVN':
        nVegClass = 11
        vprmConstants = WriteVPRMConstants.WriteVPRMConstants(outdir = datp, nveg = 11)
    else: #reduce to 8 vegetation classes (no 4 evergreens needed, one enough)
        nVegClass = 8
    ...
    for k in range(nVegClass-1):
        ...
        if landcov == 'SingleEviMap':
            ...
            vegFrac = np.repeat(vegFracMap[k][np.newaxis,:,:], nhrs, axis=0)
        else:
        ...
        if ((landcov =='DVN' and (k in [6,9]))) or (landcov != 'DVN' and (k in [3,6])):
        ...
        if (landcov == 'DVN' and ( k in [0,1,2,3])) or (landcov != 'DVN' and k == 0):
            phenologyselect = np.arange(0, evi.size)
            pScalar[:] = 1
        
        if (landcov == 'DVN' and (k in [4,5,6,8,10])) or (landcov != 'DVN' and (k in [1,2,3,5,7])):  #if decid, mixed, shrub, crop, or other
            threshmark = 0.55
        ...
        xiaoGPP = lambdaGPP*tempScalar*wScalar*pScalar*evi*radScalar*Rad*vegFrac*(-1)
